...
import com.gexin.artplatform.adapter.MyFansListAdapter;
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (fansList.get(position).getuType() == 2) {
					...
					intent.putExtra("userId", fansList.get(position).get_id());
					startActivity(intent);
				}
			}
		});
		...
		try {
			...
			if (state == 1) {
				if (type == 1) {
					...
					temp = gson.fromJson(
							jObject.getJSONArray("fans").toString(),
							new TypeToken<List<Fans>>() {
							}.getType());
				}else if(type==4){
					temp = gson.fromJson(
							jObject.getJSONArray("follows").toString(),
							new TypeToken<List<Fans>>() {
							}.getType());
				}
				if (temp == null) {
